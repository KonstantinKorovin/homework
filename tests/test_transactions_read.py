from src.main import PATH_TO_CSV, PATH_TO_XLSX
...
from tests.conftest import dict_transactions
...
def test_transactions_avg(list_transactions_fixture):
    assert transactions_avg(list_transactions_fixture) == {
        'Открытие вклада': 1, 'Перевод организации': 5, 'Перевод с карты на карту': 1, 'Перевод со счета на счет': 2
    }
